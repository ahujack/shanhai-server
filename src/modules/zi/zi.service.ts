      ...
      try {
        ...
        if (llmResult) {
          ...
          if (llmResult.lihefa?.length || llmResult.tianziGe?.length || llmResult.imageryInference) {
            ziAnalysis = {
              ...ziAnalysis,
              ...(llmResult.lihefa?.length && { lihefa: llmResult.lihefa }),
              ...(llmResult.tianziGe?.length && { tianziGe: llmResult.tianziGe }),
              ...(llmResult.imageryInference && { imageryInference: llmResult.imageryInference }),
              ...(llmResult.probingQuestion && { probingQuestion: llmResult.probingQuestion }),
            };
          }
          // 甲骨文象形：LLM 从甲骨字形出发，结合方向解读
          if (llmResult.oracleBoneInterpretation) {
            ziAnalysis = {
              ...ziAnalysis,
              oracleBone: {
                ...ziAnalysis.oracleBone,
                interpretation: llmResult.oracleBoneInterpretation,
              },
            };
          }
          ...
          if (llmResult.handwritingInterpretation) {
            ...
            handwriting = {
              ...handwriting,
              ...(hi.pressureInterpretation && { pressureInterpretation: hi.pressureInterpretation }),
              ...(hi.stabilityInterpretation && { stabilityInterpretation: hi.stabilityInterpretation }),
              ...(hi.structureInterpretation && { structureInterpretation: hi.structureInterpretation }),
              ...(hi.continuityInterpretation && { continuityInterpretation: hi.continuityInterpretation }),
            };
          }
          // 方向解读
          interpretation = {
...
【输出格式】JSON，以下字段必须全部返回：
{
            ...
            },
            {
              ...
              content: JSON.stringify({
                ...
                ziAnalysis: {
                  ...
                  oracleBone: {
                    exists: ziAnalysis.oracleBone.exists,
                    interpretation: ziAnalysis.oracleBone.interpretation,
                    imageCount: ziAnalysis.oracleBone.imageUrls?.length ?? 0,
                  },
                },
              }),
            },
  ...
  /** 婚恋方向：按五行给出差异化建议，避免千篇一律 */
  private getLoveByWuxing(wuxing: string): string {
    const map: Record<string, string> = {
      '木': '关系像生长，需要空间与耐心，不宜急于求成',
      '火': '感情热烈但易起伏，宜先稳情绪再谈承诺',
      '土': '重视稳定与责任，建议把期待说清楚，减少揣测',
      '金': '原则感强，沟通时先共情再讲理，避免硬碰硬',
      '水': '情感流动，需要安全感与确认，定期复盘关系节奏',
    };
    return map[wuxing] || '先对齐期待，再对齐行动';
  }

  /** 婚恋方向：按吉凶给出差异化策略 */
  private getLoveByJixiong(jixiong: string): string {
    const map: Record<string, string> = {
      '吉': '当前窗口适合推进关系，可主动表达或推进关键议题',
      '凶': '本阶段宜收敛情绪，先确认事实再下结论，避免冲动决定',
      '平': '稳态试探期，把误会和承诺拆开处理更有效',
    };
    return map[jixiong] || '先对齐期待，再对齐行动';
  }

  /** 婚恋方向：按字形部件联想（有「心」「女」「口」等时更贴切） */
  private getLoveByComponents(components: string[], zi: string): string {
    const hasXin = components.some((c) => /心|忄/.test(c)) || /心|忄/.test(zi);
    const hasNv = components.some((c) => /女/.test(c)) || /女/.test(zi);
    const hasKou = components.some((c) => /口/.test(c)) || /口/.test(zi);
    if (hasXin) return '你更在意内心感受的对接，建议先说出真实想法再谈行动';
    if (hasNv) return '关系中的角色与边界很重要，先厘清彼此期待再推进';
    if (hasKou) return '沟通是核心，约定一次“无打断沟通”时段会有帮助';
    return '关系关键在“先对齐期待，再对齐行动”';
  }
